...
An alternative, fully-backwards compatible syntax (parses fine even on PHP 5) closely inspired by Rust:
...
As a possible exception to the entry-point rule, the file pointed to by the first require/include statement of an entry point may also be preemptively parsed (potentially recursively) at compile time to scan for the `enable_features` statement: this would allow the autogenerated Composer autoloader to enable compile-time features through a `declare` statement on top of the autoloader.  

Given the manual entry-point requirement for the `enable_features` declare, most features should be designed to be enabled at runtime.  

Runtime-enablable features may **not** be enabled using the `enable_features` declare (to avoid syncing manually enabled features with composer-based requirements).  
